...
#Functionality of search button for conference
...
#Functionality of search button for journal
...
def dummy_jou():
    ...
    if request.method == 'POST':
        ...
        # 'SELECT * FROM login WHERE AUTHOR_ID=%s AND PASSWORD=%s', (autor_id, password))
        cur = mysql.connection.cursor()
        value=cur.execute('SELECT * FROM journal WHERE Name_of_Journal=%s', (Name_of_Journal,))
        if value>0:
            details=cur.fetchall()
...
def view_book():
    ...
    if request.method == 'POST':
        ...
        # 'SELECT * FROM login WHERE AUTHOR_ID=%s AND PASSWORD=%s', (autor_id, password))
        cur = mysql.connection.cursor()
        value=cur.execute('SELECT * FROM book_chapter1 WHERE paper_title=%s', (paper_title,))
        if value>0:
            details=cur.fetchall()
